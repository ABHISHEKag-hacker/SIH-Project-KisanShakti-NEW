      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border p-6">
        ...
        <form onSubmit={handleAddListing} className="bg-gray-50 dark:bg-gray-700/50 p-6 rounded-xl border dark:border-gray-700 space-y-4 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" placeholder="Item Name (e.g., 'Handmade Crafts', 'Used Bicycle')" value={newListing.name} onChange={e => setNewListing({ ...newListing, name: e.target.value })} className="input" required />
                <select value={newListing.type} onChange={e => setNewListing({ ...newListing, type: e.target.value as any })} className="input">
                    <option value="Crop">Goods</option>
                    <option value="Cattle">Services</option>
                </select>
                ...
                <input type="text" placeholder="Quantity (e.g., '1', '5 hours')" value={newListing.quantity} onChange={e => setNewListing({ ...newListing, quantity: e.target.value })} className="input" required />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:opacity-90 transition-all duration-300 transform active:scale-95 shadow-md hover:shadow-lg flex items-center justify-center gap-2">
